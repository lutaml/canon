...
* **Algorithm-specific output**: Different output styles for different diff
  algorithms
* **Whitespace adjacency**: Pretty-print indentation inside `:collapse` parents
  is filtered before alignment so a single delta does not cascade into
  multiple misleading "content differs" entries
