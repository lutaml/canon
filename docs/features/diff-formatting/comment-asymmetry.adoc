...
The contract is implemented in two cooperating sites:

* `Canon::Comparison::HtmlComparator#compare_fragment_children` and its
  helper `walk_fragment_children_with_realignment` — the HTML fragment
  path used by `dom_diff` and `equivalent?` for HTML inputs.
* `Canon::Comparison::XmlComparatorHelpers::ChildComparison` — the XML
  comparator path. The length-mismatch site uses
  `asymmetric_noise_explains_length_diff?` to suppress the structural
  diff when the asymmetry is fully explained by whitespace and/or
  comment nodes, and the realignment walker emits per-orphan diffs
