...
When `verbose: true` is set, `Canon::Comparison.equivalent?` returns an array of
difference objects instead of a boolean. This is useful for debugging test
failures or understanding exactly what differs between documents.
