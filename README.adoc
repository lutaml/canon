...
* **Color-coded output**: Red (normative deletions), green (normative additions), cyan
  (informative diffs with `~` symbol)
* **Namespace rendering**: Explicit namespace display in XML diffs using `ns:[uri]` or `ns:[{blank}]` format
* **Informative diff visualization**: Visually distinct cyan-colored `~` markers for differences that don't affect equivalence
