...
- `Canon::XmlParsing` — the only place that talks to engines for XML parse/serialize; node type queries answer for ANY recognized node (Nokogiri or moxml) by type, never by active backend — user-supplied Nokogiri nodes keep working under the moxml engine.
...
Known libleptris conformance gaps are tracked upstream (leptris/leptris#576/#577/#578, leptris-ruby#73 document-PI reader) and surfaced as engine-conditional `pending`s in the C14N specs plus `spec/canon/xml/engine_parity_spec.rb` — the executable gate. When upstream fixes land, the pendings flip to "pending, passed" — remove the markers then. Pretty-printers (`PrettyPrinter::Xml`, `XmlNormalized`) deliberately stay on the Nokogiri pipeline whenever available — pretty-printed bytes are canon's product and the moxml serializer does not yet match Nokogiri output (lutaml/moxml#129).
