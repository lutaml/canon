...
Known remaining engine gaps are tracked upstream and surfaced as engine-conditional `pending`s in the C14N specs plus `spec/canon/xml/engine_parity_spec.rb` — the executable gate. Fixed upstream so far (libleptris 1.9.7 / moxml 0.5.9): attribute-value normalization, prolog/epilog PIs, epilog comments at C level, batched SAX (leptris#576/#577/#578/#585). Still open: moxml#137 (adapter drops document-level comments), leptris#606 (DTD ATTLIST defaults applied by default — breaks W3C C14N ex 3.3), moxml#138 (materialize records need namespace declarations — unblocks bulk conversion; the wrapper-walk conversion is ~3x Nokogiri until then), moxml#129 (serializer byte parity — last blocker for pretty-printing on leptris), moxml#134 (Document#free). leptris SAX is at parity with Nokogiri SAX, not faster (leptris#594) — the SAX driver stays Nokogiri until that changes. Pretty-printers (`PrettyPrinter::Xml`, `XmlNormalized`) deliberately stay on the Nokogiri pipeline — pretty-printed bytes are canon's product.
